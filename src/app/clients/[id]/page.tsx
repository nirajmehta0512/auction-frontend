    ...
    const load = async () => {
      try {
        ...
        const calculatedStats: ClientStats = {
          ...
          averageInvoiceValue: invoices.length ? invoices.reduce((sum: number, i: any) => {
            const total = i.total_amount || i.amount || i.total || 0
            return sum + total
          }, 0) / invoices.length : 0,
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{client.vat_number || 'Not applicable'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Paddle Number</p>
                    <p className="font-medium text-gray-900">{client.paddle_no || 'Not assigned'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Platform</p>
                    <p className="font-medium text-gray-900">{client.platform || 'Not specified'}</p>
                  </div>
                  <div>
                  ...
                  {client.tags && (
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{client.tags}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="text-sm text-gray-700 space-y-1">
                      {client.billing_address1 && <p>{client.billing_address1}</p>}
                      ...
                      {(client.billing_city || client.billing_post_code) && (
                        <p>{[client.billing_city, client.billing_post_code].filter(Boolean).join(', ')}</p>
                      )}
                      {(client.billing_region || client.billing_country) && (
                        <p>{[client.billing_region, client.billing_country].filter(Boolean).join(', ')}</p>
                      )}
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-700 space-y-1">
                      {client.shipping_same_as_billing ? (
                        ...
                        <>
                          {client.shipping_address1 && <p>{client.shipping_address1}</p>}
                          ...
                          {(client.shipping_city || client.shipping_post_code) && (
                            <p>{[client.shipping_city, client.shipping_post_code].filter(Boolean).join(', ')}</p>
                          )}
                          {(client.shipping_region || client.shipping_country) && (
                            <p>{[client.shipping_region, client.shipping_country].filter(Boolean).join(', ')}</p>
                          )}
                  ...
                  {overview?.purchases?.length ? (
                    <div className="space-y-3">
                      {overview.purchases.slice(0, 5).map((purchase: any) => (
                        <div key={purchase.id} className="flex justify-between items-start p-3 bg-gray-50 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Link 
                      href={`/logistics?client_id=${clientId}`} 
                      className="text-sm text-teal-600 hover:text-teal-700 flex items-center"
                    >
                      <Eye className="h-4 w-4 mr-1" />
                      View All
                  ...
                  {overview?.logistics?.length ? (
                    <div className="space-y-3">
                      {overview.logistics.slice(0, 5).map((logistics: any) => (
                        <div key={logistics.id} className="flex justify-between items-start p-3 bg-gray-50 rounded-lg">
                          <div className="flex-1">
                            <p className="font-medium text-gray-900 text-sm">
                              {logistics.destination_type || 'Shipment'}
                            </p>
                            <p className="text-xs text-gray-500">
                              {logistics.created_at ? new Date(logistics.created_at).toLocaleDateString() : ''}
                            </p>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 text-sm">No logistics records</p>
                    </div>
                  )}
                </div>
              </div>
